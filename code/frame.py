...
class frame:
    ...
    def showResult(self):
        ...
        for index in xrange(0, self.size):
            ...
            image = Image.open(path_head + path_back)
            ...
            image.save('./Visualize/' + str(label) + '/' + str(index) + '.jpg',
                       'JPEG')
    ...
    def median(self, numberlist):
        ...
        if length % 2 == 1:
            ...
            return (numberlist[int(length / 2) - 1] + numberlist[int(
                length / 2)]) / 2.0
    ...
    def midAffinity(self, group_element):
        ...
        for i in xrange(0, len(group_element)):
            ...
            for j in xrange(0, len(group_element)):
                if j != i:
                    affinitylist.append(self.AffinityMatrix[group_element[i]][
                        group_element[j]])
    ...
    def Recommend(self, candidate_list, aim_list, num):
        ...
        for x in candidate_list:
            ...
            score.append(self.median(affinitylist))
        DescentIndex = sorted(
            range(len(score)), key=lambda k: score[k], reverse=True)
    ...
    def showResult_Order(self, dirname):
        ...
        for root in dict_label:
            ...
            for i in xrange(0, len(DescentIndex)):
                ...
                path = self.dataset.imageNameList[index]
                path_back = path[0]
                path_next = path_back.replace(
                    '/home/sensetime/renren/pein0119_test/',
                    '/home/heyue/Downloads/clusterQNet/data/renren_new/')
                path_next = path_next.replace(
                    '/data3/ckd/RenrenFeature/',
                    '/home/heyue/Downloads/clusterQNet/data/renren_new/')
            ...
            if root != 1 and root != 2:
                ...
                candidate, score = self.Recommend(candidate_list,
                                                  dict_label[root],
                                                  int(side * 2))
                for i in xrange(0, len(candidate)):
                    ...
                    #查询gt
                    gt = self.gt[index]
                    if gt == self.gt[dict_label[root]
                                     [DescentIndex[0]]] and gt != 1 and gt != 2:
                        patch = Rectangle(
                            (0, 0),
                            255,
                            255,
    ...
    def get_Knearest(self, x):
        ...
        DescentIndex = sorted(
            range(len(DM_Affinity)),
            key=lambda k: DM_Affinity[k],
            reverse=True)
    ...
    def Normalize_label(self):
        ...
        for root_i in dict_group_Q:
            dict_average_Q[root_i] = sum(dict_group_Q[root_i]) / len(
                dict_group_Q[root_i])
            #设定侧脸阈值
