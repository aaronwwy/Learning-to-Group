    ...
    for i in range(nc):
        for j in range(i + 1, nc):
            ...
            affinity[i][j] = (1 / Ci**2) * np.transpose(ones_i).dot(W_ij).dot(
                W_ji).dot(ones_i) + (1 / Cj**2) * np.transpose(ones_j).dot(
                    W_ji).dot(W_ij).dot(ones_j)
    ...
    while currentClusterNum > targetClusterNum:
        ...
        for i in range(len(neighborSet)):
            ...
            if max_index1 in neighborSet[i]:
                aff_update = getAffinityBtwCluster(cluster[i],
                                                   cluster[max_index1], W)
        ...
        for i in range(len(neighborSet[max_index1])):
            ...
            newAffinity = getAffinityBtwCluster(cluster[target_index],
                                                cluster[max_index1], W)
